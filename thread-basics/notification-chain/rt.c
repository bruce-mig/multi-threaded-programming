...
void rt_init_rt_table(rt_table_t *rt_table) { rt_table->head = NULL; }

rt_entry_t *rt_add_or_update_rt_entry(rt_table_t *rt_table, char *dest,
...
void rt_table_register_for_notification(rt_table_t *rt_table,
   ...
   if (!rt_entry) {
       ...
       * */
      rt_entry =
          rt_add_or_update_rt_entry(rt_table, key->dest, key->mask, 0, 0);
