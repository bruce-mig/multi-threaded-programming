...
static void *_udp_server_create_and_start(void *arg) {
   ...
   while (1) {
      ...
      bytes_recvd = recvfrom(udp_sock_fd, recv_buffer, MAX_PACKET_BUFFER_SIZE,
...
pthread_t *udp_server_create_and_start(char *ip_addr, uint32_t udp_port_no,
   ...
   pthread_create(recv_pkt_thread, &attr, _udp_server_create_and_start,
                  (void *)thread_arg_pkg);

   return recv_pkt_thread;
}

int send_udp_msg(char *dest_ip_addr, uint32_t dest_port_no, char *msg,
                 uint32_t msg_size) {
   ...
   int rc = sendto(sock_fd, msg, msg_size, 0, (struct sockaddr *)&dest,
